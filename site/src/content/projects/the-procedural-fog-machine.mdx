...
1. User input enters through the app interface: the scary message, optional context, domain, stance, and slop density.
2. The UI calls a dedicated outline route, `/api/cannon/outline`, which sends the request through OpenRouter.
3. The outline model defaults to `x-ai/grok-4.3`, with deployment configuration available through `OPENROUTER_OUTLINE_MODEL`.
4. The slop density slider determines the section count, target words, and section token budget.
5. Each outline section is generated through `/api/cannon/section-stream`, using OpenRouter streaming and the configured section model.
6. The UI reads streamed NDJSON chunks and assembles them into the live packet preview as sections arrive.
7. If slop density is set to 11, the app sends each completed section through `/api/cannon/inflate` to generate a supplemental addendum.
...
The quality of the output depends on model choice, but also on how the system breaks the task apart, routes each stage, streams progress, and preserves a clear document shape for the user. The implementation is deliberately modular: outline generation, streamed section generation, fallback section generation, and density-11 inflation each live behind separate API routes.
