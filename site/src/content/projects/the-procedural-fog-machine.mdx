...
<a href="https://procedural-fog-machine.vercel.app" target="_blank" rel="noopener noreferrer">Try the live demo</a>
...
- Next.js app deployed on Vercel, with the interface, generation controls, live log, and packet preview handled in the client.
- Natural-language input form for the dispute, optional source context, domain, stance, and slop density.
- Split API routes for each generation stage: one route creates the outline, another streams section bodies, and a separate max-density route expands completed sections.
- OpenRouter for model routing, so outline generation and section expansion can use different models without changing the product code.
- Grok for high-level packet outlining, where the model's job is to plan the structure before generation begins.
- DeepSeek v4 Flash Nitro for streamed section writing in production, optimized for fast, low-cost longform expansion.
- Streaming response handling that converts section output into incremental chunks the UI can assemble into the live document preview.
- Density planning logic that maps the slop slider to section count, target length, and token budget.
- A density-11 expansion pass that sends completed sections back through the section-writing model to generate supplemental addenda.
