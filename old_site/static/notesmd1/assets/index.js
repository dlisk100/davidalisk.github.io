...
var MemStorage = class {
  ...
  currentId;
  constructor() {
    this.demoRequests = /* @__PURE__ */ new Map();
    this.currentId = 1;
  }
  async createDemoRequest(request) {
    const id = this.currentId++;
    const demoRequest = { ...request, id };
...
var demoRequests = pgTable("demo_requests", {
  ...
  numProviders: text("num_providers").notNull(),
...
function registerRoutes(app2) {
  app2.post("/api/demo-request", async (req, res) => {
    try {
      ...
      const result = await storage.createDemoRequest(data);
      res.json(result);
    } catch (error) {
